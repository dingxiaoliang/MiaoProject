...
6. 市面上的电视不同，各种设置也不同，我能测试的也只是我本地的设备，至于能否开启杜比视界 / 杜比全景声，请不要太纠结
- 开启杜比视界的条件
  1. 「仅加载最高分辨率」：关闭
  2. 「过滤超过60帧的视频」：关闭
  3. 「匹配视频动态范围和帧率」：全部开启
...
## 关于杜比全景声
- 总结：虽然B站提供了「杜比全景声」的编码，但是因为压制封装的问题，目前无法在 Apple TV 上点亮「杜比全景声」
- 🔍 杜比全景声无法激活的根本原因，文件确实包含 Atmos 元数据，但有以下关键问题：
  1. Complexity Index 缺失，**Complexity index: Not present / 16**，Complexity Index 是播放器判断是否需要激活 Atmos 渲染的重要标识。这里显示 "Not present"，说明该字段没有被正确写入，部分解码器会因此退回到普通 DD+ 5.1 模式。
  2. Bed 声道配置异常，**Bed channel count: 1 channel**，**Bed channel configuration: LFE（仅低频）**，正常 Atmos 内容的 Bed 层通常是 5.1 或 7.1。这里 Bed 只有 LFE 一个声道，非常不寻常。实际的 L/R/C/Ls/Rs 声道都被当作 Object 处理了（Number of dynamic objects: 15）。这种配置虽然合规，但兼容性极差，很多设备/播放器无法正确解析。
  3. 容器 compatible_brands 问题，**compatible_brands: isomdby1iso2mp41iso5**，包含了 dby1（Dolby 专有 brand），理论上是好事，但 encoder: Lavf59.27.100 表明这是用 FFmpeg 5.x 封装的。FFmpeg 在写入 E-AC-3 JOC 时，对 Atmos 相关的 box（dec3/ChannelMap）支持不完整，容易导致激活失败。
  4. 声道数量报告为 6（5.1），而非 object-based，**channels: 6 / channel_layout: 5.1(side)**，ffprobe 把它识别为普通 5.1，说明 JOC（Joint Object Coding）扩展信息对 ffprobe 来说是透明的——这同样意味着某些播放器也可能无法识别 Atmos 对象层。
